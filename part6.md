...
- `List<Int>?` : 전체 리스트가 널이 될 수 있다. (리스트 안에는 널 불가)
- `List<Int?>?`: 널이 될 수 있는 값으로 이뤄진 널이 될 수 있는 리스트

### 6.3.2 읽기 전용과 변경 가능한 컬렉션
- 코틀린은 컬렉션 안의 데이터에 접근하는 인터페이스와 컬렉션 안의 데이터를 변경하는 인터페이스를 분리했음
- `Collection` 인터페이스는 원소에 대해 인터레이션하고 컬렉션 데이터를 읽을 수 있지만 원소를 추가하거나 제거할 수 없다.
- `MutableCollection` 인터페이스는 데이터를 수정할 때 사용 (`Collection` 인터페이스를 확장)
    - 가급적 읽기 전용 인터페이스를 사용하는것을 원칙으로 삼아라 

### 6.3.3 코틀린 컬렉션과 자바
![image](https://github.com/simjunbo/kotlin/assets/7076334/af6b02af-1b3b-4a87-b859-89f5b865e66f)
